...
#### Step A — Trigger (INPUT)

Two trigger modes:

| Mode | Source | Behavior |
|------|--------|----------|
| **Auto** | New email in Outlook folder `{{OUTLOOK_INTAKE_FOLDER}}` | Email body → passed to Operator 01 |
| **Manual** | User pastes disruption notice text in Run dialog | Pasted text → passed to Operator 01 (no Outlook involved) |
...
| Manual trigger | Accept paste disruption notice text (not email) |

> On manual trigger, Operator 01 extracts fields directly from the pasted text — no Outlook email is involved. case_key is generated by Operator 01, not by the trigger.
