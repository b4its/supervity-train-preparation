# Prompt: Procurement Exception Decision Rules

```text
Outcome: Create configurable, testable decision rules for the Procurement Exception Commander so business users can change thresholds, approval routing, and templates without editing operator prompts.
...
Hard governance overrides. These always route to HIGH and review_required=true:
...
Score other cases transparently. Return a score breakdown rather than one opaque score:
- direct line value at risk above HIGH_VALUE_THRESHOLD_MYR: +30
- direct line value at risk above MEDIUM_VALUE_THRESHOLD_MYR: +15
...
Routes:
- LOW: no hard override and score <= LOW_ROUTE_MAX_SCORE. Monitoring-only recommendation permitted. Human review is optional only if no human action or external communication is proposed.
- MEDIUM: no hard override and score <= MEDIUM_ROUTE_MAX_SCORE. Human review required because a procurement owner must act.
...
Create at least five rule tests: a low monitoring case, a medium operational case, a sole-source case, a penalty-clause case, and a missing-data case. Present the rule and tests for approval before publishing.
```
